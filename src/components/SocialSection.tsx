...
const mockPosts = [
  ...
  { id: 4, emoji: "⚡", caption: "Hydration, reimagined. What does your body deserve? 💧", likes: "4.2k", bg: "linear-gradient(135deg, var(--color-cream-beige), #EDE5D8)", border: "rgba(14,90,67,0.12)", tall: true },
  { id: 5, emoji: "🍵", caption: "Natural tea extracts. Khandsari sugar & stevia. Electrolytes. That's it. 🌾", likes: "2.9k", bg: "linear-gradient(135deg, #FFFBEA, var(--color-cream-beige))", border: "rgba(182,139,94,0.25)", tall: false },
...
export default function SocialSection() {
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(180deg, #F0EBE0 0%, var(--color-ivory) 100%)", padding: "96px 0" }}
    >
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 right-0 w-56 h-56 rounded-full blur-3xl opacity-10" style={{ background: "var(--color-peach-orange)" }} />
        <div className="absolute bottom-0 left-0 w-56 h-56 rounded-full blur-3xl opacity-10" style={{ background: "var(--color-citrus-yellow)" }} />
      </div>
      ...
      <div className="section-container relative z-10">
        ...
        >
          ...
          <p className="text-forest-green/55 max-w-md mx-auto text-sm md:text-base mb-8">
          ...
          <motion.a
            ...
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-semibold text-ivory"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-14" style={{ gridAutoRows: "160px" }}>
          {mockPosts.map((post, i) => (
            ...
            >
              ...
              {/* Hover overlay */}
              <div className="absolute inset-0 bg-forest-green/88 flex flex-col items-center justify-center gap-2 p-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
                {igIcon}
                <p className="text-white text-xs text-center leading-snug mt-1">{post.caption}</p>
                <div className="flex items-center gap-1 mt-1">
                  <span className="text-peach-orange text-xs">♥</span>
                  <span className="text-sage-green text-xs">{post.likes}</span>
                </div>
              </div>
            </motion.a>
          ))}
        ...
        <motion.div
          ...
          style={{
            background: "linear-gradient(135deg, var(--color-forest-green) 0%, var(--color-botanical-green) 50%, #0a3d2e 100%)",
            padding: "56px 40px",
          }}
          initial={{ opacity: 0, y: 24 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
        ...
        >
          ...
          />
          <p className="text-sage-green text-xs tracking-[0.4em] uppercase mb-4">Join the Community</p>
          <h3
            className="font-playfair font-bold text-ivory mb-3"
            style={{ fontSize: "clamp(1.5rem, 4vw, 2.25rem)" }}
          >
            Join the hydration movement.
          </h3>
          <p className="text-sage-green text-xs tracking-[0.3em] uppercase font-semibold mb-4">
            Your Daily Reset Drink
          </p>
          <p className="text-sage-green text-sm mb-8 max-w-sm mx-auto leading-relaxed">
          ...
          <motion.a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-ivory text-forest-green font-semibold text-sm"
